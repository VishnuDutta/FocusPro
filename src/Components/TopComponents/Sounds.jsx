...
import audioFile from "/src/assets/Sounds/audioFile.mp3";
import YoutubeThumbnail from "./SoundSpaces/YoutubeThumbnail";
import Youtube from "./SoundSpaces/Youtube";
...
const singleSoundLibrary = {
  ...
  brownNoiseSound: {
    name: "BrownNoiseSound",
    audio: audioFile,
    // audio: new Audio(audioFile),
  },
  greenNoiseSound: {
    name: "GreenNoiseSound",
    audio: audioFile,
    // audio: new Audio(audioFile),
  },
  pinkNoiseSound: {
  ...
  cabinNoiseSound: {
    name: "CabinNoiseSound",
    audio: audioFile,
    // audio: new Audio(audioFile),
  },
  rainonsurfaceSound: {
    name: "RainonSurfaceSound",
    audio: audioFile,
    // audio: new Audio(audioFile),
  },
  rainSound: {
  ...
  bettleSound: {
    name: "BettleSound",
    // audio : new Audio("/src/assets/Sounds/insects.mp3"),
    // audio: new Audio(audioFile),
    svgElement: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
  ...
  airconditionerSound: {
    name: "AirconditionerSound",
    audio: audioFile,
    // audio: new Audio(audioFile),
  },
  mechanicalkeyboardSound: {
    name: "MechanicalKeyboardSound",
    audio: audioFile,
    // audio: new Audio(audioFile),
    svgElement: (
      <svg
...
const soundSpacesLibrary = {
  rainfall: {
    ...
    soundsToPlay: ["musicSound", "cloudSound", "boatSound", "wavesSound"],
  },
  noise: {
    name: "Noise",
    backgroundImage:
    ...
    soundsToPlay: ["musicSound", "birdsSound", "splashSound", "bubblesSound"],
  },
  campfire: {
    name: "CampFire",
    backgroundImage:
  ...
  garden: {
    ...
    soundsToPlay: ["oceanSound", "rainSound", "seagullsSound", "windSound"],
  },
  cafe: {
    name: "Cafe",
    backgroundImage:
  ...
  thunderstrom: {
    name: "ThunderStrom",
    backgroundImage:
      "https://th.bing.com/th/id/OIP.OiyuiYkhEDGILdBilaQANgHaE7?cb=iwc2&rs=1&pid=ImgDetMain",
    svgElement: (
      <svg
        data-v-51ce07eb=""
        fill="currentColor"
...
const youtubeLinks = [
  "https://youtu.be/Z8IRRphKFZA?si=lIb9tcoJREyOhHoZ",
  "https://youtu.be/vsWxs1tuwDk?si=_SPEWpppp9xkwb_z",
  "https://youtu.be/RLvOIeYXobE?si=LR4XUGFgPhhidUdg",
  "https://youtu.be/BXNxrT59MzQ?si=zx2aolZjIdFEOO_k",
  "https://youtu.be/fRJ03btNsao?si=tajfavvALca8jrem",
  "https://youtu.be/c-FKlE3_kHo?si=5dvCLW8G37Yt-EjN",
];

export default function Sounds() {
  //All states starts from here

  //State for showing and displaying div's

  // ---------------------
  const [showdiv, setShowdiv] = useState(true);
  const [masterPlayPause, setMasterPlayPause] = useState(false);

  //this just show the Active State of 4 Buttons
  ...
  //State to manages single sounds - playingstatus , volumecontrol , showSound or not

  const [singleSoundsPlayingStatus, setSingleSoundsPlayingStatus] = useState(
    () => {
      return Object.keys(singleSoundLibrary).reduce((acc, key) => {
        acc[key] = {
          showSound: false,
          playingStatus: false,
          volume: 0.5,
  ...
  //this State manages the soundSpaces playing Status

  const [soundSpacesPlayingStatus, setSoundSpacesPlayingStatus] = useState(
    () => {
      return Object.keys(soundSpacesLibrary).reduce((acc, key) => {
        acc[key] = false; //default : not playing
        return acc;
      }, {});
    }
  );

  //this state manage the youtube sound is playing

  const [youtubePlayingStatus, setYoutubePlayingStatus] = useState(() => {
    return youtubeLinks.reduce((acc, key) => {
      acc[key] = false;
      return acc;
    }, {});
  });
  ...
  const currentSoundSpacePlaying = useRef(null);
  const audioObjectRef = useRef({});
  const playersRef = useRef([]);

  //All useEffect

  useEffect(() => {
    audioObjectRef.current = Object.entries(singleSoundLibrary).reduce(
      (acc, [soundKey, soundData]) => {
  ...
  useEffect(() => {
    ...
    Object.values(audioObjectRef.current).forEach(({ audio }) => {
      audio.volume = masterVolume;
    });

    // Update all YouTube player volumes
    playersRef.current.forEach((player) => {
      if (player && typeof player.setVolume === "function") {
        player.setVolume(masterVolume * 100); // YouTube volume is 0–100
      }
    });
  }, [masterVolume]);
  ...
  function updateBottomDivs(divname) {
    const keysExtract = Object.keys(soundCenterActiveDiv);
    const falseKeys = createFalseState(keysExtract);

    setSoundCenterActiveDiv((prev) => ({
      ...falseKeys,
      [divname]: true,
    }));

    //Exception for Custom Button
  }

  //function to update the playing status of each sound

  const eachSoundPlayingtoogle = (soundId) => {
    setSingleSoundsPlayingStatus((prev) => ({
      ...prev,
      [soundId]: {
        ...prev[soundId],
        playingStatus: !prev[soundId].playingStatus,
      },
    }));
  };

  // function to update the volume of the each sound
  ...
  //function to update soundSpaces playingStatus

  //when ever clicking to Sound Space this function is toggling

  const soundSpacesPlayingToogle = (soundSpaceName) => {
    const isSameSpace = currentSoundSpacePlaying.current === soundSpaceName;

    setSoundSpacesPlayingStatus((prev) => {
      const updatedStatus = Object.keys(prev).reduce((acc, key) => {
        acc[key] = false;
        return acc;
      }, {});
      // updatedStatus[soundSpaceName] = !prev[soundSpaceName];

      if (!isSameSpace) {
        updatedStatus[soundSpaceName] = true;
        currentSoundSpacePlaying.current = soundSpaceName;
      } else {
        currentSoundSpacePlaying.current = null;
      }
      return updatedStatus;
    });

    const selectedSpace =
      soundSpacesLibrary[soundSpaceName]?.soundsToPlay || [];

    setSingleSoundsPlayingStatus((prev) => {
      const updatedStatus = Object.entries(prev).reduce(
        (acc, [soundID, soundData]) => {
          acc[soundID] = {
            ...soundData,
            showSound: !isSameSpace && selectedSpace.includes(soundID),
            playingStatus: !isSameSpace && selectedSpace.includes(soundID),
          };
          return acc;
        },
        {}
    ...
    updateBottomDivs("customDiv");
  };

  //function to play or pause the Sound According to the input

  function playSounds(soundname) {
    const audio = audioObjectRef.current[soundname]?.audio;
    if (audio) {
      audio.play();
    }
  }
  ...
  //Pausing all others youtube Players
  const handleVideoPlayStatus = (currentPlayer) => {
    playersRef.current.forEach((player) => {
    ...
    // Register the current player if not already in the list
    if (!playersRef.current.includes(currentPlayer)) {
      playersRef.current.push(currentPlayer);
    }
  };

  const handleYoutubePlaying = (url) => {
    setYoutubePlayingStatus((prev) => {
      //first wanted to set all status to false then wanted to set all status to true
      ...
      updatedStatus[url] = !prev[url];
      return updatedStatus;
    });

    updateBottomDivs("nowPlayingDiv");
  };

  return (
    <div className="absolute top-4 ml-5">
        ...
        <div className="absolute bg-black/80 z-2 ml-2 rounded-2xl mt-3 p-5 text-white flex flex-col gap-5 font-bold w-120 sm:w-120 lg:w-200 max-h-250 overflow-x-auto">
          ...
          <div className="soundsCenter flex gap-4 justify-between">
            <ul className="flex gap-1">
              <li>
                <button
                  onClick={() => updateBottomDivs("soundspacesDiv")}
                  className={`${BtnCss} ${
              ...
              <li>
                <button
                  onClick={() => updateBottomDivs("youtubeDiv")}
                  className={`${BtnCss} ${
              ...
              <li>
                <button
                  onClick={() => updateBottomDivs("customDiv")}
                  className={`${BtnCss} ${
            ...
            <ul className="nowPlaying">
              <li>
                <button
                  onClick={() => updateBottomDivs("nowPlayingDiv")}
                  className={`${BtnCss} ${
          ...
          {soundCenterActiveDiv.soundspacesDiv && (
            <div className="soundSpaces">
              <ul className="grid grid-cols-3 lg:grid-cols-5 gap-2">
                {Object.entries(soundSpacesLibrary).map(
                  ([soundSpaceName, soundSpaceData]) => (
                    <SingliSoundSpaceLi
                      key={soundSpaceName}
                      src={soundSpaceData.backgroundImage}
                      mixPlayingStatus={
                        soundSpacesPlayingStatus[soundSpaceName]
                      }
                      name={soundSpaceData.name}
                      svgElement={soundSpaceData.svgElement}
                      callBack={() => soundSpacesPlayingToogle(soundSpaceName)}
                    />
                  )
                )}
              </ul>
          ...
          {soundCenterActiveDiv.youtubeDiv && (
            <div>
              <ul className="grid grid-cols-3 lg:grid-cols-4 gap-2">
                {youtubeLinks.map((url) => {
                  const thumbID = url.split("youtu.be/")[1].split("?")[0];
                  return (
                    <YoutubeThumbnail
                      key={thumbID}
                      thumbID={thumbID}
                      playingStatus={youtubePlayingStatus[url]}
                      callBack={() => handleYoutubePlaying(url)}
                    />
                  );
                })}
              </ul>
          ...
          {soundCenterActiveDiv.customDiv && (
            <>
              <ul className="grid grid-cols-5">
                {Object.entries(singleSoundLibrary).map(
                  ([soundID, soundData]) => (
                    <SingliSoundSpaceAudio
                      key={soundID}
                      soundId={soundID}
                      name={soundData.name}
                      svgElement={soundData.svgElement}
                      playingStatus={
                        singleSoundsPlayingStatus[soundID].playingStatus
                      }
                      displayStatus={
                        singleSoundsPlayingStatus[soundID].showSound
                      }
                      volumeLevel={singleSoundsPlayingStatus[soundID].volume}
                      onVolumeChange={updateVolume}
                      onPlayingStatusChange={eachSoundPlayingtoogle}
                    />
                  )
                )}
              </ul>
          ...
          {youtubeLinks.map((url, index) => {
            ...
            return (
              <div
                key={index}
                style={{
                  display: youtubePlayingStatus[url] ? "block" : "none",
                }}
              >
                <Youtube
                  videoUrl={thumbID}
                  videoPlayHandle={handleVideoPlayStatus}
                  playingStatus={youtubePlayingStatus[url]}
                />
              </div>
            );
