...
export default function SingliSoundSpaceAudio({
  name,
  //soundID helps to change the playing status
  soundId,
  //svg element gives the SVG data
  ...
  return (
    <li
      title={name}
      className={`relative flex flex-col w-20 h-20 mx-auto group ${
        playingStatus ? "opactity-100" : "opacity-50"
      }
      ${displayStatus ? "" : "hidden"}`}
      onClick={()=> onPlayingStatusChange(soundId)}
    >
      {svgElement}
      <input
        type="range"
        ...
        step={0.01}
        onChange={(e) => {
          onVolumeChange(soundId , parseFloat(e.target.value))
        }}
      />
    </li>
  );
